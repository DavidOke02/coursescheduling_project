...
public class CourseListViewController {
    ...
    public void createCourseTable(){
        ...
        try {
            ...
            createUserTable.executeUpdate("CREATE TABLE IF NOT EXISTS Course (" +
                    ...
                    "  FOREIGN KEY (`professor_id`) REFERENCES coursescheduling_db.Professor (`id`)," +
                    "  PRIMARY KEY (`id`))");
    ...
    public void displayCourseList(String... deptCode) {  // Filtering option for department
        Connection connection = DBConnection.getConnection();

        try {
            ResultSet resultSet;
            if (deptCode.length >= 1) {
                PreparedStatement displayFilteredCourseList = connection.prepareStatement("SELECT * FROM coursescheduling_db.Course WHERE department_code = (?)");
                ...
                PreparedStatement displayCourseList = connection.prepareStatement("SELECT * FROM coursescheduling_db.Course");
                resultSet = displayCourseList.executeQuery();
                System.out.println("Displaying Course List...");
            }

            DefaultTableModel tableModel = (DefaultTableModel) view.getTable().getModel();
            tableModel.setRowCount(0);  // Resetting table rows before adding new data
            ...
            Object[] rowdata = new Object[6];  // Array size updated to accommodate the timeslot
            ...
            while (resultSet.next()) {
                try {
                    Course course = new Course(
                            ...
                            resultSet.getString("timeslot")  // Retrieve the timeslot field
                    );
                    // Add course to table
                    ...
                    rowdata[5] = course.getTimeslot();  // Display the timeslot
